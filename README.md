...
A continuación se presenta la documentación de la API central de ThePackCo, se trata de una API REST que recibe y envía información en formato JSON.
...
Este corresponde al entorno de producción. No existe un entorno de pruebas independiente, pero se entregará dos tokens de autenticación diferentes: uno para hacer las pruebas y otro para usar de forma definitiva.
...
Recibe la información del envío en el _request body_ (formato JSON). En el recurso [order](#order) se detallan los atributos que se pueden entregar al momento de crear el envío.
...
**IMPORTANTE**: cuando obtenemos la información de un envío, el atributo `address_city` contiene el código de la comuna. Cuando creamos un envío, el atributo `address_city` debe contener el nombre de la comuna.
...
Si la información proporcionada tiene errores, devuelve código de error (`400`) y un objeto de errores. En cada entrada del objeto de errores, la llave corresponde al nombre del campo que tiene el error y el valor corresponde a una lista de los errores asociados a ese campo.
...
- `401`: el token de autenticación no es válido.
- `403`: el token de autenticación no tiene permisos para consultar este recurso.
- `404`: no existe una tienda con el `:shop_id` entregado, o es una tienda a la que el usuario no tiene acceso, o no existe el envío con el `:order_id` entregado.

#### Respuestas

En el caso de una respuesta positiva (`200`) retorna el envío. Ver recurso [Order](#order) para conocer todos los atributos.
...
Al crear un envío, deben entregarse obligatoriamente los atributos marcados con el símbolo :exclamation:, y opcionalmente los con el símbolo :question:. Por otra parte, los atributos que tienen el símbolo :book: son atributos de solo lectura y por lo tanto solo pueden ser leídos al obtener la información de un envío.
